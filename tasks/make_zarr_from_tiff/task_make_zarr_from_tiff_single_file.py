    ...
    def worker_function(self, block):
        ...
        for num in sections:
            ...
            if i.shape != (self.y_tile_size, self.x_tile_size):
                raise RuntimeError(f"{tif} is malformed. shape = {i.shape} does not match {(self.y_tile_size, self.x_tile_size)}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == 'run_worker':
        ...
        ap.add_argument("--single_file_format", type=int, help='Override error when there are missing tiffs. Better to put manually missing/bad sections using the bad_sections argument', default=0)
