...
import daisy
import re
from evaluation_matrix import find_merge_errors, find_split_errors, \
    rand_voi_split_merge
...
def compare_segmentation_to_ground_truth_skeleton(
    ...
    colours = color_generator(len(segmentation_paths),
                              ...
                              configs['output']['colormaps'])

    parameters = [(agglomeration_thresholds, seg_path, num_processes,
                   configs['skeleton']) for seg_path in segmentation_paths]
    ...
    for i, seg_path in enumerate(segmentation_paths):
        ...
        for graph in graph_list:
            if graph is None:
                ...
                error_configs = configs['error_count']
                output_configs = configs['output']
                split_errors, _ = find_split_errors(
                    graph, error_configs['ignore_glia'],
                    error_configs['max_break_size'])
                merge_errors, _ = find_merge_errors(
                    graph, error_configs['z_weight_multiplier'],
                ...
                if output_configs['write_TXT'] or output_configs['write_CSV']:
                    ...
                    merge_error_rows, split_error_rows = format_errors(
                        merge_errors, split_errors, graph,
                        output_configs['voxel_size'])
                    output_path, file_name = generate_output_path_and_file_name(
                        output_configs, seg_vol, seg_path)
...
def color_generator(length, colours, colormaps):
    random.seed(3)
    print(colormaps[0])
    more_colour_vals = []
    # color per colormap
    cpc = 5
    # colormaps = [cm for cm in plt.colormaps() if not cm.endswith("_r")]
    more_colours = []
    subractor = 0
    while(len(more_colours) < length):
        i = len(more_colour_vals)

        cyc_pos = i % cpc
        r = cyc_pos * (1 / cpc)

        more_colour_vals.append(r)
        if cyc_pos == 0:
            print(int(i / cpc))
            cmap = cm.get_cmap(colormaps[int(i / cpc)])
        color = cmap(r)
        color_hls = cs.rgb_to_hls(*mc.to_rgb(color))
        # make sure only saturated and dark colors are used
        if(color_hls[1] < 0.8) and (color_hls[2] > 0.3):
            more_colours.append(cs.hls_to_rgb(*color_hls))
        # elif color_hls[2] <= 0.3 and color_hls[1] <0.8:
        # more_colours.append
        # (cs.hls_to_rgb(*(color_hls[0], color_hls[1] , 0.35)))
        else:
            more_colours.append(cs.hls_to_rgb(*(color_hls[0], r, color_hls[2])))

    # print(more_colour_vals)
    default_rgb_arrays = [mc.to_rgb(c) for c in colours]
    default_colours = np.array(default_rgb_arrays)
    rea = np.append(default_colours, more_colours, axis=0)
    # print(rea)

    return(more_colours)


def generate_graphs_with_seg_labels(agglomeration_thresholds,
                                    segmentation_path,
                                    num_processes, skeleton_configs):
    graph_list = []
    unlabelled_skeleton = construct_skeleton_graph(
        skeleton_configs['skeleton_path'],
        skeleton_configs['with_interpolation'],
        skeleton_configs['step'], skeleton_configs['leaf_node_removal_depth'])

    if os.path.exists(os.path.join(segmentation_path, 'luts/fragment_segment')):
        fragment_graph = add_predicted_seg_labels_from_vol(
            unlabelled_skeleton.copy(), segmentation_path, 'volumes/fragments',
            skeleton_configs['load_segment_array_to_memory'])

        parameters_list = [(fragment_graph.copy(), segmentation_path,
                           'volumes/' + threshold)
                           for threshold in agglomeration_thresholds]

        graph_list = [replace_fragment_ids_with_LUT_values(
            *pl) for pl in parameters_list]
    else:
        parameters_list = [(unlabelled_skeleton.copy(), segmentation_path,
                            'volumes/' + threshold,
                            skeleton_configs['load_segment_array_to_memory'])
                           for threshold in agglomeration_thresholds]
        graph_list = [add_predicted_seg_labels_from_vol(
            *pl) for pl in parameters_list]
...
def generate_error_plot(
    ...
    for j in range(int(len(split_and_merge) / 3)):
        ...
        for a, b, m, l in zip(split_and_merge[j * 3 + 1],
                              split_and_merge[j * 3 + 2],
    ...
    output_file_name = output_path + '/' + \
        config_file_name + '_' + volume_name + '_' + error_metric
...
def format_errors(merge_errors, split_errors, graph, voxel_size):
    ...
    for error in merge_errors:
        ...
        coord1 = str(to_pixel_coord_xyz(
            graph.nodes[node1]['zyx_coord'], voxel_size))
        coord2 = str(to_pixel_coord_xyz(
            graph.nodes[node2]['zyx_coord'], voxel_size))
        merge_error_rows.append(
            ['M', segment_id, coord1, coord2, node1, node2])
    for error in split_errors:
        ...
        coord2 = str(to_pixel_coord_xyz(
            graph.nodes[node2]['zyx_coord'], voxel_size))
        split_error_rows.append(
            ['S', skeleton_id, coord1, coord2, node1, node2])
