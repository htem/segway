...
from task_05_graph_evaluation_print_error import color_generator, \
...
# clean this method up and/or consider reformatting the config JSONs and
# task_defaults
...
def run_evaluation(
    ...
    if 'Inputs' in config:
        ...
        splits_and_merges = [format_splits_and_merges(
            x) for x in zip(*splits_and_merges)]

        print("Summed splits and merges:")
        print(splits_and_merges)

        #use par_configs here
        ...
        generate_error_plot(config['Input']['segment_dataset'],
                            ...
                            color_generator(len(volume['segment_volumes']),
                                            config['Output']
                                            ['colors'], config['Output']['colormaps']),
                            config['Output']['font_size'],
                            config['Output']['line_width'], 'number',
                            *splits_and_merges)
    else:
        parameter_configs = format_parameter_configs(
            config, config['Input'], 0)
...
# Takes in dictionary, and returns a list of splits and merges ordered by
# volume
