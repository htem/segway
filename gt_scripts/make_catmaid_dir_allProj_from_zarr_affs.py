...
catmaid_f_xy = config["CatmaidOut"]["dir"] + "/" + catmaid_folder + "_affs/xy"
catmaid_f_xz = config["CatmaidOut"]["dir"] + "/" + catmaid_folder + "_affs/xz"
catmaid_f_zy = config["CatmaidOut"]["dir"] + "/" + catmaid_folder + "_affs/zy"
...
for z_index in range(z_begin, z_end):
    for y_index in range(y_begin, y_end):
        for x_index in range(x_begin, x_end):
            ...
            tile = Image.blend(img_affs, tile, alpha)
...
for y_index in range(y_begin, y_end):
    for z_index in range(z_begin, z_end):
        for x_index in range(x_begin, x_end):
            ...
            print(slice_roi)
            slice_array = cutout_ds[slice_roi].to_ndarray().reshape(
                    int(slice_roi_shape[0]/voxel_size[0]),
                    int(slice_roi_shape[2]/voxel_size[2]))
